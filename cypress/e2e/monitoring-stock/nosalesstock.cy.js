...
describe('Low Stock', () => {
    ...
    it('Low Stock API return 400', () => {
        ...
        cy.location('hash').should('eq', '#/stock/not-sold');
        cy.intercept('GET', '/api/category?merchant_id=1&page=1&limit=999999999999&store_id=3', { fixture: 'category' }).as('getCategory-Fixture');
        cy.intercept('GET', '/api/stock/nosales?page=1&merchant_id=1&limit=15&name=&store_id=3', {
            statusCode: 400,
            body: {
                success: false,
                error: 'Bad request',
    ...
    it('Low Stock API return 401', () => {
        ...
        cy.wait(500);
        cy.location('hash').should('eq', '#/login?redirect=%2Fstock%2Fnot-sold');
    });
});
